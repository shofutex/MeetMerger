...
It also allows us to clean up heat sheets generated by other tools to meet our team's requirements. 

The input heat sheet should be structured as follows (PDF) or [using the sample data](https://github.com/shofutex/MeetMerger/blob/main/test-data/sample_heat_sheet.pdf). The second line is the team acronym, name, year, and time for a record-holder for the event. This tool allows one record per team per event. There is no limit to the number of teams this tool will support, but it's only been tested up to 6.

**#1 Boys 6 & Under 25m Freestyle**\
**ST First Last           Year   TIME**\
**Heat 1 of 1**\
1 Swimmer one &emsp;&emsp;&emsp;Age&emsp;Team&emsp;Entry time

Or as follows for [CSV](https://github.com/shofutex/MeetMerger/blob/main/test-data/sample_heat_sheet.csv):
```
